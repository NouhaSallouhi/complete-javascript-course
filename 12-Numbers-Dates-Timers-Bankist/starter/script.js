...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    // When 0 secs, stop the timer and log out user, to be really logged out after showing 0 sec, time-- must comes after this
    if (time === 0) {
      clearInterval(timer);
  ...
  tick(); // not waiting 1 sec but run immediately
  const timer = setInterval(tick, 1000); // and the every 1 sec
  return timer; // to avoid 2 timers running together
};

///////////////////////////////////////
// Event handlers
let currentAccount, timer; // to set timer as global variable bc to proceed with different logins
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    // Start logout timer at login and if sb already logs in, stop his timer and start new timer
    if (timer) clearInterval(timer);
    timer = startLogOutTimer();
    // timer here and timer in starLogOutTimer() is different
...
const options = {
  ...
  unit: 'mile-per-hour',
  currency: 'EUR',
  // useGrouping: false, // no , or . to separate numbers
};
...
const pizzaTimer = setTimeout(
  ...
  // 'olive',
  // 'salami'
); // arguments come as 3rd param
console.log('waiting...'); // logs first

// To stop timer (can be stopped before reaching delay)
...
//   const stunde = `${jetzt.getHours()}`.padStart(2, '0');
//   const minute = `${jetzt.getMinutes()}`.padStart(2, '0');
//   const sekunde = `${jetzt.getSeconds()}`.padStart(2, '0');
//   console.log(`Now it's ${stunde}:${minute}:${sekunde}`);
// }, 1000);
*/
